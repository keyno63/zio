 ...
 *
 * The "before" side of migratecatseffect.Step8TimeAndRetry. Only sleep/ timeout
 * are shown here — retry policies live in the separate cats-retry library, not
 * cats-effect itself, so there is no compiled retry snippet to mirror ZIO's
 * Schedule-based retry.
 *
 * sbt "migrate-cats-effect/runMain
 * migratecatseffect.catseffect.Step8TimeAndRetry"
 */
