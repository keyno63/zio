...
/**
 * Guide: Migrate from Cats Effect to ZIO Section: Typing Your Error Channel
 *
 * Replaces: IO.raiseError(e) -> ZIO.fail(e) io.handleErrorWith(f) ->
 * zio.catchAll(f) io.recover { case ... } -> zio.catchSome { case ... }
 * io.attempt -> zio.either (no equivalent) -> zio.mapError(f)
 *
